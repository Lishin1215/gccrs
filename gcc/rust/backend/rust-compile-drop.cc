	  ...
	  if (ctx->lookup_drop_flag (it->hirid, &flag))
	    {
	      ...
	      tree clear = drop_builder.drop_flag_assignment (
		it->hirid, false, it->locus);
	      tree guarded_drop
		= Backend::statement_list ({clear, drop_stmt});
	      drop_stmt = Backend::if_statement (ctx->peek_fn ().fndecl,
						 condition, guarded_drop,
						 NULL_TREE, it->locus);
	    }
