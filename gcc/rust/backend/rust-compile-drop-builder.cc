			     ...
			     bool initialized)
{
  ...
  Bvariable *flag
    = Backend::temporary_variable (ctx.peek_fn ().fndecl, nullptr,
				   boolean_type_node,
				   Backend::boolean_constant_expression (
				     initialized),
				   false, locus, &declaration);
...
DropBuilder::drop_flag_assignment (HirId hirid, bool value, location_t locus)
{
  ...
  return Backend::assignment_statement (
    Backend::var_expression (flag, locus),
    Backend::boolean_constant_expression (value), locus);
}
